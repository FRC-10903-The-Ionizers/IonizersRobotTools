...
public class ArcadeDrive extends Command {
  ...
  public void execute() {
    ...
    final var xSpeed = m_xspeedLimiter.calculate(MathUtil.applyDeadband(y, 0.1))
        * 0.1; //DriveSubsystem.k_maxDriveSpeedMetersPerSecond;

    // Get the y speed or sideways/strafe speed. We are inverting this because
    // we want a positive value when we pull to the left. Xbox controllers
    // return positive values when you pull to the right by default.
    final var ySpeed = m_yspeedLimiter.calculate(MathUtil.applyDeadband(x, 0.1))
        * 0.1; //DriveSubsystem.k_maxDriveSpeedMetersPerSecond;
    ...
    double rot = m_rotLimiter.calculate(MathUtil.applyDeadband(turn, 0.2))
